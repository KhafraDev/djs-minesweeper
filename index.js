class Minesweeper {
    ...
    getBombs() {
        ...
        this.bombs = [];
        for(let i = 0; i < this.rows; i++) { // row
            for(let j = 0; j < this.columns; j++) { // column
        ...
        for(const [r, c] of this.bombs) {
            ...
            if(typeof this.mineArray[r-1] !== 'undefined' && typeof this.mineArray[r-1][c-1] !== 'undefined' && this.mineArray[r-1][c-1] !== 9) this.mineArray[r-1][c-1]++;
            if(typeof this.mineArray[r-1] !== 'undefined' && typeof this.mineArray[r-1][c+1] !== 'undefined' && this.mineArray[r-1][c+1] !== 9) this.mineArray[r-1][c+1]++;
            if(typeof this.mineArray[r+1] !== 'undefined' && typeof this.mineArray[r+1][c+1] !== 'undefined' && this.mineArray[r+1][c+1] !== 9) this.mineArray[r+1][c+1]++;
            if(typeof this.mineArray[r+1] !== 'undefined' && typeof this.mineArray[r+1][c-1] !== 'undefined' && this.mineArray[r+1][c-1] !== 9) this.mineArray[r+1][c-1]++;
        }
        ...
        for(let i = 0; i < this.rows; i++) { // row
            for(let j = 0; j < this.columns; j++) { // column
                ...
                switch(this.mineArray[i][j]) {
                    ...
                    default: val = '|| :bomb: ||'; break;
                }
                this.mineArray[i][j] = val;
            }
        }

        while(true) { // change one hidden zero to starter hint
